...
export function initializeEID(eid : EIDInterface): void
{
	// #region 11 new items!
        ...
        eid.addCollectible(CollectibleTypeLTT.DIPLOPTIONS, "All pickup options no longer disappear when picking up its companion collectible.");
        ...
        eid.addCollectible(CollectibleTypeLTT.OBSESSION, `Averages out all consumables rounded up then adds +2 bombs, +2 keys and +2 coins#Grants +3 damage and +3 luck when all consumables are equal.#Grants +1 damage and +1 luck when 2 consumables are equal.#With {{Trinket${TrinketType.EQUALITY}}} Equality, consumables lower than your highest consumable count with have their pickups doubled.`);
    // #endregion
    ...
    // #region Plagues of Egypt
        ...
        eid.addCollectible(CollectibleTypeLTT.PLAGUE_OF_LOCUSTS, `Random locusts are spawned when an enemy is killed.#Locusts will consume any pickup they collide with.#When colliding with a pickup the locust has a chance on being upgraded to a permanent {{Collectible${CollectibleType.ABYSS}}} Abyss locust.`);
